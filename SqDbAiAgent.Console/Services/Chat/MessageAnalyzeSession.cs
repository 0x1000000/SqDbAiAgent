...
public sealed class MessageAnalyzeSession
{
    ...
    public async Task<MessageClassificationResponse?> ClassifyAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        for (var attempt = 1; attempt <= this._appConfig.MaxClassificationAttempts; attempt++)
        {
            var reply = await this.TryChatJsonAsync(
                ...
                currentInstruction,
                MessageClassificationResponse.JsonSchema,
    ...
    public async Task<NewTopicClassificationResponse?> CheckNewTopicAsync(
        IReadOnlyList<ChatMessage> oldMessages,
        string newMessage,
        CancellationToken cancellationToken = default)
    {
        ...
        for (var attempt = 1; attempt <= this._appConfig.MaxClassificationAttempts; attempt++)
        {
            var reply = await this.TryChatJsonAsync(
                ...
                currentInstruction,
                NewTopicClassificationResponse.JsonSchema,
                "new-topic detection",
                attempt,
                cancellationToken);
            if (reply is null)
    ...
    private static string BuildClassificationSystemPrompt(string databaseName, string schemaPrompt) =>
        $$"""
         You are the message classifier for an assistant connected to a database.
    ...
    private static string BuildNewTopicSystemPrompt(string databaseName, string schemaPrompt) =>
        $$"""
         You are the topic-continuity checker for an assistant connected to a database.
         Return exactly one complete JSON object:
         {"isNewTopic":true}
    ...
    private static MessageClassificationResponse? TryParseClassification(string reply)
    {
        var trimmed = StripMarkdownFence(reply);
        return MessageClassificationResponse.TryParseFromJson(trimmed, out var result)
            ? result
            : null;
    }

    private static NewTopicClassificationResponse? TryParseNewTopicCheck(string reply)
    {
        var trimmed = StripMarkdownFence(reply);
        return NewTopicClassificationResponse.TryParseFromJson(trimmed, out var result)
            ? result
            : null;
    }
