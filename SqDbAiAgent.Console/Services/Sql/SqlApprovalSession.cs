...
public sealed class SqlApprovalSession : ISqlApprovalSession
{
    ...
    private readonly ChatHistoryManager<SqlRepairResponse> _conversationHistory;
        ...
        string databaseName)
    {
        ...
        this._conversationHistory = new ChatHistoryManager<SqlRepairResponse>(
        ...
        while (attempt < this._appConfig.MaxSqlFixAttempts)
        {
                ...
                && this.TryApproveSql(currentSql, out var parsedExpression, out approvalError))
            {
                var removedCount = this._conversationHistory.Push(
                    userRequest,
                    new SqlRepairResponse(SqlRepairResponseType.TSqlCode, currentSql));
                if (removedCount > 0)
                {
                    this._output.OutDebugLine($"SQL fixer history trimmed. Removed {removedCount} old turn(s).");
                }

                var limited = SqlQueryLimiter.ApplyDefault(
                    (IExprReadOnlyQuery)parsedExpression!,
                    this._appConfig.DefaultQueryRowLimit);
                return SqlApprovalResult.Approved(
                    limited.Applied ? limited.Query.ToSql(TSqlExporter.Default) : currentSql,
                    limited.Query,
                    limited.Applied);
            }
    ...
    private async Task<SqlRepairResponse?> TryGetSqlFixResponseAsync(
        string userRequest,
        string currentSql,
        string error,
        string errorKind,
        CancellationToken cancellationToken)
    {
        ...
        for (var attempt = 1; attempt <= this._appConfig.MaxFixResponseAttempts; attempt++)
        {
            ...
            if (response is { } fixResponse && fixResponse.RespType == SqlRepairResponseType.TSqlCode)
            {
        ...
        out string error)
    {
        ...
        if (expr is not IExprReadOnlyQuery)
        {
    ...
    private static string BuildSqlFixSystemPrompt(string databaseName, string schemaPrompt)
    {
        return
            $@"
You repair and approve Microsoft SQL Server queries for the connected database.
...
"
    ...
    private static SqlRepairResponse? TryParseResponse(string reply)
    {
        ...
        return SqlRepairResponse.TryParseFromJson(trimmed, out var response)
            ? response
            : null;
    }
}
